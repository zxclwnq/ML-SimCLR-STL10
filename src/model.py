...
class SimCLR(nn.Module):
    ...
    def __init__(self, out_dim: int = 128) -> None:
        ...
        # for STL-10, which has 96x96 images, we need to adjust the first conv
        ...
        # 2-Layers MLP (Linear -> BN -> ReLU -> Linear -> BN) as it is in repository of original paper
        self.projector = nn.Sequential(
            nn.Linear(features_dim, features_dim, bias=False),
            nn.BatchNorm1d(features_dim),
            nn.ReLU(inplace=True),
            nn.Linear(features_dim, out_dim, bias=False),
